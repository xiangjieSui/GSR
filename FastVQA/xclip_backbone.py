...
class XCLIP(CLIP):
    ...
    def forward_original(self, image, text):
        ...
        text_features = text_features + \
            self.prompts_generator(text_features, img_features)
        ...
        logit_scale = self.logit_scale.exp()
        logits = torch.einsum("bd,bkd->bk", video_features,
                              logit_scale * text_features)

        return logits


def build_x_clip_model(pretrained_path="./model/k400_32_8.pth", droppath=0., use_checkpoint=False, logger=None, prompts_alpha=1e-1, prompts_layers=2, use_cache=True, mit_layers=4, **kwargs):
