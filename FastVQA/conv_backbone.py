...
class Block3D(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., inflate_len=3, layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv3d(dim, dim, kernel_size=(inflate_len, 7, 7), padding=(
            inflate_len // 2, 3, 3), groups=dim)  # depthwise conv
        self.norm = LayerNorm(dim, eps=1e-6)
        # pointwise/1x1 convs, implemented with linear layers
        self.pwconv1 = nn.Linear(dim, 4 * dim)
        self.act = nn.GELU()
...
class ConvNeXt3D(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        self.downsample_layers = nn.ModuleList()
        stem = nn.Sequential(
            nn.Conv3d(in_chans, dims[0], kernel_size=(
                2, 4, 4), stride=(2, 4, 4)),
            LayerNorm(dims[0], eps=1e-6, data_format="channels_first")
        )
        self.downsample_layers.append(stem)
    ...
    def inflate_weights(self, s_state_dict):
        ...
        for key in t_state_dict.keys():
            ...
            if t_state_dict[key].shape != s_state_dict[key].shape:
                ...
                s_state_dict[key] = s_state_dict[key].unsqueeze(
                    2).repeat(1, 1, t, 1, 1) / t
...
def convnext_3d_small(pretrained=False, in_22k=False, **kwargs):
    model = ConvNeXt3D(depths=[3, 3, 27, 3], dims=[
                       96, 192, 384, 768], **kwargs)
    if pretrained:
        url = model_urls['convnext_small_22k'] if in_22k else model_urls['convnext_small_1k']
        checkpoint = torch.hub.load_state_dict_from_url(
            url=url, map_location="cpu", check_hash=True)
...
if __name__ == "__main__":
    ...
    # print(
    #     profile(model, (torch.randn(4, 3, 32, 224, 224).to(device),))[0] / 1e9)
